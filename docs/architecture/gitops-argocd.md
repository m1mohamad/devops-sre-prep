...
Continuously compare reviewed Git intent with live Kubernetes objects and expose drift. The boundary must remain diagnosable when a dependency is slow, unavailable, unauthorized, or returning stale state.
...
flowchart LR
  ...
  Edge --> Control[GitOps with Argo CD control]
