...
Give Pods routable identities and stable service discovery while enforcing explicit policy. The boundary must remain diagnosable when a dependency is slow, unavailable, unauthorized, or returning stale state.
...
flowchart LR
  ...
  Edge --> Control[Kubernetes Networking control]
