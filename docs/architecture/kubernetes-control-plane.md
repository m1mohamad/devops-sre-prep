...
Accept declarative intent durably and converge it without coupling clients to node execution. The boundary must remain diagnosable when a dependency is slow, unavailable, unauthorized, or returning stale state.
...
flowchart LR
  ...
  Edge --> Control[Kubernetes Control Plane control]
