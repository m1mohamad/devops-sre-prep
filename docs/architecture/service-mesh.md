...
Apply workload identity and selected traffic policy between services while pricing dataplane cost. The boundary must remain diagnosable when a dependency is slow, unavailable, unauthorized, or returning stale state.
...
flowchart LR
  ...
  Edge --> Control[Service Mesh control]
