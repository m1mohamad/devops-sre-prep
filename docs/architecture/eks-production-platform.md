...
Operate multi-AZ workloads across controlled node, identity, traffic, and upgrade boundaries. The boundary must remain diagnosable when a dependency is slow, unavailable, unauthorized, or returning stale state.
...
flowchart LR
  ...
  Edge --> Control[EKS Production Platform control]
