...
tags: [platform-engineering, software-delivery]
aliases: [Software Delivery handbook]
---
...
A build creates an artifact once; environments promote its digest. Deployment changes runtime state, while release controls user exposure. A production design states ownership, capacity, security, failure behavior, and a reversible change path instead of treating tooling as the outcome.

## Control and request flow

```mermaid
flowchart LR
  Intent[Reviewed intent] --> Control[Software Delivery control plane]
...
Trace the real chain as **commit, test evidence, immutable digest, provenance, promotion intent**. At every transition ask: what identity is used, where durable state lives, whether the operation is idempotent, which timeout bounds it, and how status distinguishes accepted from completed work.
