...
aliases: [Observability handbook]
---
...
Metrics aggregate, logs preserve events, and traces connect causality. OpenTelemetry transports signals; it does not decide an SLO or guarantee useful instrumentation. A production design states ownership, capacity, security, failure behavior, and a reversible change path instead of treating tooling as the outcome.

## Control and request flow

```mermaid
flowchart LR
  Intent[Reviewed intent] --> Control[Observability control plane]
...
Trace the real chain as **instrumentation, collector, backend, query, decision**. At every transition ask: what identity is used, where durable state lives, whether the operation is idempotent, which timeout bounds it, and how status distinguishes accepted from completed work.
