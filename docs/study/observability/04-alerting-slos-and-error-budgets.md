...
An SLO measures a user-valued SLI over a window. Error budget quantifies tolerated bad events; multi-window burn alerts detect fast and slow exhaustion with actionable routing.
...
```mermaid
flowchart LR
  Requests --> SLI --> SLOWindow --> ErrorBudget --> BurnAlert --> OnCall
```
...
Define eligible population and good event precisely. Page on symptoms with a runbook and ownership; use ticket alerts for slower risk. Budget policy changes release/risk decisions but is not punishment.
...
|---|---|---|
| noisy page | low action rate | change threshold/routing |
| blind spot | customer reports without SLI | improve measurement |
| budget gaming | denominator change | govern definition |
...
Describe a real **Alerting, SLOs, and Error Budgets** decision: quantify the constraint and impact, name the decisive evidence, explain the rejected alternative, and identify the durable guardrail and owner.
