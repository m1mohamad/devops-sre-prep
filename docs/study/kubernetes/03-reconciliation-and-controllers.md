...
Controllers turn declarative desired state into eventual observed state. Informers list once, watch changes into a shared cache, call lightweight handlers that enqueue keys, and workers reconcile idempotently with rate-limited retries.
...
```mermaid
flowchart LR
  API -->|initial LIST / WATCH| Informer --> Cache
  Cache --> Handler --> KeyQueue[rate-limited key queue]
  KeyQueue --> Workers -->|get/patch status| API
```

## Core Components

resourceVersion orders observations, generation changes with spec, observedGeneration and conditions report processed intent. Owner references drive garbage collection; finalizers delay deletion for controller cleanup.

## How It Actually Works

Handlers do not perform external work: they enqueue namespace/name, allowing duplicate-key coalescing. Multiple workers read possibly stale cache state and must tolerate NotFound/conflict. Reconcile reads current desired/observed state, makes safe repeated changes, patches status and forgets success or rate-limits transient failure. Leader election prevents multiple active controller replicas where required; optimistic concurrency prevents lost updates.
...
|---|---|---|
| poison key | retries/queue age | surface terminal condition |
| stale cache | resourceVersion/conflict | retry idempotently |
| stuck finalizer | deletionTimestamp/finalizer age | repair external cleanup |
...
Describe a real **Reconciliation and Controllers** decision: quantify the constraint and impact, name the decisive evidence, explain the rejected alternative, and identify the durable guardrail and owner.
