...
aliases:
  - GitOps: Argo CD and Flux study note
---
...
```mermaid
flowchart LR
  Intent[Reviewed intent] --> Control[GitOps: Argo CD and Flux control]
...
Many workloads must be placed, restarted, networked, and updated without operators hand-editing individual hosts. GitOps: Argo CD and Flux provides a repeatable boundary for that problem. Standardization enables policy and automation, but the abstraction must retain escape hatches and debuggable underlying resources.
...
Describe a production change involving GitOps: Argo CD and Flux. What constraint selected the design? Name the first signal, the misleading signal, the rollback decision, and one durable improvement. Quantify blast radius or recovery time without inventing business results.

## Recall Check

1. What is the consumer-facing contract for GitOps: Argo CD and Flux?
...
* [Next focused note](09-service-mesh-istio-linkerd-cilium.md)
