...
aliases: [Performance Investigation]
---
...
Decompose latency into queue, application, dependency and network time under controlled load; optimize the measured bottleneck and retest.

## Mental Model

Treat the observable boundary as part of the design. Coordinate omission and missing warm-up mislead tests. Watch p95/p99, saturation, errors and throughput; a faster component may only move the bottleneck.

## Architecture Diagram

```mermaid
flowchart LR
  LoadGenerator --> Queue --> App --> Database --> Response
```
...
Decompose latency into queue, application, dependency and network time under controlled load; optimize the measured bottleneck and retest. Coordinate omission and missing warm-up mislead tests. Watch p95/p99, saturation, errors and throughput; a faster component may only move the bottleneck.
...
```bash
wrk; curl -w; pidstat 1; iostat -xz 1; perf stat
```
...
Coordinate omission and missing warm-up mislead tests. Watch p95/p99, saturation, errors and throughput; a faster component may only move the bottleneck.
...
Describe one production decision involving **Performance Investigation**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
