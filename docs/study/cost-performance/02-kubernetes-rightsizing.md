...
aliases: [Kubernetes Rightsizing]
---
...
Requests drive scheduling and node scale; limits enforce cgroup ceilings. Rightsizing balances bin packing, tail latency, OOM risk and disruption headroom.

## Mental Model

Treat the observable boundary as part of the design. Use percentile plus known bursts, not average alone. CPU limits can throttle; memory limits kill. Preserve PDB and zonal headroom.

## Architecture Diagram

```mermaid
flowchart LR
  Metrics --> Requests --> Scheduler --> Nodes --> Autoscaler
```
...
Requests drive scheduling and node scale; limits enforce cgroup ceilings. Rightsizing balances bin packing, tail latency, OOM risk and disruption headroom. Use percentile plus known bursts, not average alone. CPU limits can throttle; memory limits kill. Preserve PDB and zonal headroom.
...
```bash
kubectl top pod -A --containers; kubectl describe node; kubectl get vpa -A
```
...
Use percentile plus known bursts, not average alone. CPU limits can throttle; memory limits kill. Preserve PDB and zonal headroom.
...
Describe one production decision involving **Kubernetes Rightsizing**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
