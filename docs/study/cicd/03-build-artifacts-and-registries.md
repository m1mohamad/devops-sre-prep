...
Build once, address by digest and promote the tested bytes. A registry stores OCI manifests/blobs; a multi-architecture index selects platform manifests. SBOM, provenance and signature answer composition, build origin and authorization separately.
...
```mermaid
flowchart LR
  Source --> Builder --> OCIIndex --> Registry
  Builder --> SBOM
  Builder --> Provenance --> Signature
```
...
Make tags immutable where possible, scan the final filesystem, verify runtime architecture, retain known-good digests and apply lifecycle policy without deleting rollback evidence.
...
|---|---|---|
| wrong architecture | manifest index/runtime error | restore digest |
| mutable tag | digest differs | block mutation |
| scanner outage | gate status | use declared risk policy |
...
Describe a real **Build Artifacts and Registries** decision: quantify the constraint and impact, name the decisive evidence, explain the rejected alternative, and identify the durable guardrail and owner.
