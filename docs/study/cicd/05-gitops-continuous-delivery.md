...
Git records reviewed deployment intent; a cluster-side reconciler renders, compares and applies it. Artifact registry remains the source of deployable bytes and application data remains outside Git.
...
flowchart LR
  Source --> CI --> Registry
  CI --> GitOpsRepo --> Reconciler --> Cluster
```
...
Separate sync and health, assign field ownership, bound prune and hooks, scope tenant destinations and define break-glass. Revert Git to a known digest for declarative rollback.
...
|---|---|---|
| drift fight | managedFields | assign owner |
| bad prune | audit/diff | restore resource |
| Git outage | last healthy data plane | pause changes |
...
Describe a real **GitOps Continuous Delivery** decision: quantify the constraint and impact, name the decisive evidence, explain the rejected alternative, and identify the durable guardrail and owner.
