---
title: Processes, Memory, CPU, and I/O
tags: [linux-networking, interview-prep]
aliases: [Processes, Memory, CPU, and I/O]
---

# Processes, Memory, CPU, and I/O

## 30-Second Answer

CPU saturation, scheduler delay, memory pressure and storage latency have different evidence; load average alone cannot identify them.

## Mental Model

Treat the observable boundary as part of the design. High iowait calls for device and workload analysis, while OOM requires working-set, cgroup and reclaim evidence—not more CPU.

## Architecture Diagram

```mermaid
flowchart LR
  Process --> Scheduler --> CPU; Process --> Pages --> Memory; Process --> Files --> Disk
```
...
CPU saturation, scheduler delay, memory pressure and storage latency have different evidence; load average alone cannot identify them. High iowait calls for device and workload analysis, while OOM requires working-set, cgroup and reclaim evidence—not more CPU.
...
```bash
top -H; free -m; vmstat 1; iostat -xz 1; pidstat -dur 1; cat /proc/$PID/limits
```
...
High iowait calls for device and workload analysis, while OOM requires working-set, cgroup and reclaim evidence—not more CPU.
...
Describe one production decision involving **Processes, Memory, CPU, and I/O**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
