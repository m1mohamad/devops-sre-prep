---
title: Linux Troubleshooting Method
tags: [linux-networking, interview-prep]
aliases: [Linux Troubleshooting Method]
---
...
Start from user impact, establish a time window, then move from service to process, host and network evidence without restarting away the clues.

## Mental Model

Treat the observable boundary as part of the design. A restart can mitigate but destroys transient evidence; capture logs, sockets, limits and pressure first.

## Architecture Diagram

```mermaid
flowchart LR
  Client --> systemd --> Process --> Kernel --> Disk
```
...
Start from user impact, establish a time window, then move from service to process, host and network evidence without restarting away the clues. A restart can mitigate but destroys transient evidence; capture logs, sockets, limits and pressure first.
...
```bash
systemctl status; journalctl --since; ps -eo pid,ppid,stat,%cpu,%mem,cmd; dmesg -T
```
...
A restart can mitigate but destroys transient evidence; capture logs, sockets, limits and pressure first.
...
Describe one production decision involving **Linux Troubleshooting Method**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
