---
title: Production Network Debugging
tags: [linux-networking, interview-prep]
aliases: [Production Network Debugging]
---
...
Compare a failing and healthy path at DNS, route, handshake and application layers, capturing packets at both sides when ownership is unclear.

## Mental Model

Treat the observable boundary as part of the design. Conntrack exhaustion, packet loss, latency, asymmetric routing and SNAT each require different evidence and remediation.

## Architecture Diagram

```mermaid
flowchart LR
  Client --> DNS --> NAT --> LoadBalancer --> Service
```
...
Compare a failing and healthy path at DNS, route, handshake and application layers, capturing packets at both sides when ownership is unclear. Conntrack exhaustion, packet loss, latency, asymmetric routing and SNAT each require different evidence and remediation.
...
```bash
ss -tanp; ip -s link; conntrack -S; tcpdump -ni any host 10.0.0.8; dig; curl --connect-timeout 2
```
...
Conntrack exhaustion, packet loss, latency, asymmetric routing and SNAT each require different evidence and remediation.
...
Describe one production decision involving **Production Network Debugging**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
