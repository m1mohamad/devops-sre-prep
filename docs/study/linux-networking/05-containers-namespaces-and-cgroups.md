...
aliases: [Containers, Namespaces, and cgroups]
---
...
A container is a process isolated by namespaces and constrained/accounted by cgroups; the kernel is shared.

## Mental Model

Treat the observable boundary as part of the design. PID, mount, user and network namespaces change views; cgroup OOM and throttling can fail a container while the host looks healthy.

## Architecture Diagram

```mermaid
flowchart LR
  Runtime --> Namespaces --> Process; cgroup --> CPU; cgroup --> Memory
```
...
A container is a process isolated by namespaces and constrained/accounted by cgroups; the kernel is shared. PID, mount, user and network namespaces change views; cgroup OOM and throttling can fail a container while the host looks healthy.
...
```bash
nsenter -t $PID -n ip addr; cat /proc/$PID/cgroup; crictl inspect; ip link
```
...
PID, mount, user and network namespaces change views; cgroup OOM and throttling can fail a container while the host looks healthy.
...
Describe one production decision involving **Containers, Namespaces, and cgroups**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
