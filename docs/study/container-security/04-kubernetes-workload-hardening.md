...
aliases: [Kubernetes Workload Hardening]
---
...
Apply Pod Security Standards, scoped service accounts, secret delivery and network policy as layered controls with tested workload compatibility.

## Mental Model

Treat the observable boundary as part of the design. Restricted policy expects non-root, seccomp and no privilege escalation. Secrets in environment may leak through diagnostics; prefer mounted/rotated delivery where supported.

## Architecture Diagram

```mermaid
flowchart LR
  Admission --> PodSecurity --> Pod --> ServiceAccount; NetworkPolicy --> Pod
```
...
Apply Pod Security Standards, scoped service accounts, secret delivery and network policy as layered controls with tested workload compatibility. Restricted policy expects non-root, seccomp and no privilege escalation. Secrets in environment may leak through diagnostics; prefer mounted/rotated delivery where supported.
...
```bash
kubectl auth can-i --as system:serviceaccount:ns:app; kubectl get networkpolicy -A
```
...
Restricted policy expects non-root, seccomp and no privilege escalation. Secrets in environment may leak through diagnostics; prefer mounted/rotated delivery where supported.
...
Describe one production decision involving **Kubernetes Workload Hardening**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
