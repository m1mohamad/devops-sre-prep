---
title: SBOM, Signing, and Provenance
tags: [container-security, interview-prep]
aliases: [SBOM, Signing, and Provenance]
---
...
An SBOM identifies components; provenance attests how an artifact was built; a signature binds an identity to its digest. Each answers a different trust question.

## Mental Model

Treat the observable boundary as part of the design. Use short-lived keyless identity where suitable, protect transparency/audit evidence and enforce digest verification at admission—not mutable tags.

## Architecture Diagram

```mermaid
flowchart LR
  Build --> SBOM; Build --> Provenance --> Sign --> Registry --> Verify
```
...
An SBOM identifies components; provenance attests how an artifact was built; a signature binds an identity to its digest. Each answers a different trust question. Use short-lived keyless identity where suitable, protect transparency/audit evidence and enforce digest verification at admission—not mutable tags.
...
```bash
syft image; cosign verify; cosign verify-attestation
```
...
Use short-lived keyless identity where suitable, protect transparency/audit evidence and enforce digest verification at admission—not mutable tags.
...
Describe one production decision involving **SBOM, Signing, and Provenance**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
