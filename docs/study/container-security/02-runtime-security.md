...
aliases: [Container Runtime Security]
---
...
Runtime hardening limits what compromise can do using non-root identity, dropped capabilities, seccomp, AppArmor/SELinux and read-only filesystems.

## Mental Model

Treat the observable boundary as part of the design. Drop ALL then add only proven capabilities, prohibit privileged/host namespaces and monitor denied syscalls before tightening profiles.

## Architecture Diagram

```mermaid
flowchart LR
  Pod --> seccomp --> Syscalls; Pod --> Capabilities --> Kernel
```
...
Runtime hardening limits what compromise can do using non-root identity, dropped capabilities, seccomp, AppArmor/SELinux and read-only filesystems. Drop ALL then add only proven capabilities, prohibit privileged/host namespaces and monitor denied syscalls before tightening profiles.
...
```bash
kubectl get pod -o yaml; kubectl auth can-i; crictl inspect
```
...
Drop ALL then add only proven capabilities, prohibit privileged/host namespaces and monitor denied syscalls before tightening profiles.
...
Describe one production decision involving **Container Runtime Security**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
