---
title: PostgreSQL Performance and Connections
tags: [databases, interview-prep]
aliases: [PostgreSQL Performance and Connections]
---
...
Start with wait events and query plans: connection exhaustion, lock blocking, bad cardinality estimates and missing indexes are distinct bottlenecks.

## Mental Model

Treat the observable boundary as part of the design. Tune queries and pool budgets before raising max_connections; vacuum/analyze maintains visibility and statistics needed by the planner.

## Architecture Diagram

```mermaid
flowchart LR
  Pool --> Session --> Planner --> Executor --> Index
```
...
Start with wait events and query plans: connection exhaustion, lock blocking, bad cardinality estimates and missing indexes are distinct bottlenecks. Tune queries and pool budgets before raising max_connections; vacuum/analyze maintains visibility and statistics needed by the planner.
...
```bash
EXPLAIN (ANALYZE, BUFFERS); select * from pg_stat_statements; select * from pg_stat_activity
```
...
Tune queries and pool budgets before raising max_connections; vacuum/analyze maintains visibility and statistics needed by the planner.
...
Describe one production decision involving **PostgreSQL Performance and Connections**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
