---
title: Database Incident Scenarios
tags: [databases, interview-prep]
aliases: [Database Incident Scenarios]
---

# Database Incident Scenarios

## 30-Second Answer

Evidence-first response separates PostgreSQL pool exhaustion and lock queues from Redis eviction, hot-key and replica-lag incidents.

## Mental Model

Treat the observable boundary as part of the design. Mitigate connection storms at the pool/admission layer, cancel only identified blockers, and protect Redis origins during cache loss.

## Architecture Diagram

```mermaid
flowchart LR
  API --> Pool --> PostgreSQL; API --> Cache --> Redis
```
...
Evidence-first response separates PostgreSQL pool exhaustion and lock queues from Redis eviction, hot-key and replica-lag incidents. Mitigate connection storms at the pool/admission layer, cancel only identified blockers, and protect Redis origins during cache loss.
...
```bash
pg_isready; psql -c "select wait_event,count(*) from pg_stat_activity group by 1"; redis-cli INFO stats
```
...
Mitigate connection storms at the pool/admission layer, cancel only identified blockers, and protect Redis origins during cache loss.
...
Describe one production decision involving **Database Incident Scenarios**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
