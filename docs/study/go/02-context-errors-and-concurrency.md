---
title: Context, Errors, and Concurrency
tags: [go, interview-prep]
aliases: [Context, Errors, and Concurrency]
---
...
Context carries cancellation and deadlines across calls; it is not a configuration bag. Goroutines require bounded ownership and a shutdown path.

## Mental Model

Treat the observable boundary as part of the design. Bound concurrency, close channels from senders, select on context, preserve error identity with errors.Is, and drain or cancel workers.

## Architecture Diagram

```mermaid
flowchart LR
  Context --> errgroup --> Workers --> Channel --> Cancel
```
...
Context carries cancellation and deadlines across calls; it is not a configuration bag. Goroutines require bounded ownership and a shutdown path. Bound concurrency, close channels from senders, select on context, preserve error identity with errors.Is, and drain or cancel workers.
...
```bash
go test -race ./...; go test -run TestTimeout ./...
```
...
Bound concurrency, close channels from senders, select on context, preserve error identity with errors.Is, and drain or cancel workers.
...
Describe one production decision involving **Context, Errors, and Concurrency**: quantify impact, show the evidence that changed your mind, and name the durable guardrail.
