...
```mermaid
flowchart LR
  Intent[Reviewed intent] --> Control[IaC Mental Model control]
...
Automation increases consistency but can amplify a bad decision quickly. Strong isolation reduces blast radius but adds cost and operational surfaces. Rich abstractions speed common work but obscure internals during unusual failures. Adopt IaC Mental Model when repeated demand and risk justify a supported product; avoid adding another control plane for a one-off workload that a simpler managed service can satisfy.
...
Describe a production change involving IaC Mental Model. What constraint selected the design? Name the first signal, the misleading signal, the rollback decision, and one durable improvement. Quantify blast radius or recovery time without inventing business results.

## Recall Check

1. What is the consumer-facing contract for IaC Mental Model?
...
* [Next focused note](02-terraform-state-and-backends.md)
